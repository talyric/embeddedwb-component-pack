...
type
  TForm1 = class(TForm)
    ...
    procedure EWBProgressChange(ASender: TObject; Progress,
      ProgressMax: Integer);
    procedure EWBQueryService(Sender: TObject; const rsid,
    ...
    procedure EWBMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure EWBKeyDown(Sender: TObject; Key, ScanCode: Word;
      Shift: TShiftState);
    procedure EWBKeyUp(Sender: TObject; Key, ScanCode: Word;
      Shift: TShiftState);
  private
  ...
  case Command of
    CSC_NAVIGATEBACK:
      begin
        ...
        LogEvent(ASender, 'OnCommandStateChange', 'CSC_NAVIGATEBACK', BoolToStr(Enable));
      end;
    CSC_NAVIGATEFORWARD:
      begin
        ToolbtnForward.Enabled := Enable;
        LogEvent(ASender, 'OnCommandStateChange', 'CSC_NAVIGATEFORWARD', BoolToStr(Enable));
      end;
    CSC_UPDATECOMMANDS:
      begin
        ToolbtnStop.Enabled := Enable;
        LogEvent(ASender, 'OnCommandStateChange', 'CSC_UPDATECOMMANDS', BoolToStr(Enable));
      end;
  end;
end;
...
procedure TForm1.EWBQueryService(Sender: TObject; const rsid,
  iid: TGUID; var Obj: IInterface);
begin
...
procedure TForm1.EWBKeyDown(Sender: TObject; Key, ScanCode: Word;
  Shift: TShiftState);
begin
    LogEvent(Sender, 'OnKeyDown', Char(Key), Inttostr(ScanCode));
end;

procedure TForm1.EWBKeyUp(Sender: TObject; Key, ScanCode: Word;
  Shift: TShiftState);
begin
