...
{$IFDEF UNICODE}
function SendEMailByMAPI(SenderName, SenderAddress, Subject, Body: string; Recipients, Attachments, AttachmentNames: TStrings; WithOpenMessage, ResolveNames, NeedReceipt: Boolean; intMAPISession:
  Integer): Integer; overload;
{$ENDIF UNICODE}
function SendEMailByMAPI(SenderName, SenderAddress, Subject, Body: Ansistring; Recipients, Attachments, AttachmentNames: TStrings; WithOpenMessage, ResolveNames, NeedReceipt: Boolean; intMAPISession:
  Integer): Integer; overload;
...
{$IFDEF UNICODE}
      lpAttachArray := TlpAttachArray(AnsiStrAlloc(nFileCount * SizeOf(TMapiFileDesc)));
{$ELSE}
...
{$ENDIF UNICODE}
      ...
      begin
        ...
        lpAttachArray^[i].lpszPathName := StrPCopy(new(TlpszPathname)^, AnsiString(Attachments[i]));
        if i < AttachmentNames.Count then
        begin
          lpAttachArray^[i].lpszFileName := StrPCopy(new(TlpszFileName)^, AnsiString(AttachmentNames[i]))
        end else
        begin
          lpAttachArray^[i].lpszFileName := StrPCopy(new(TlpszFileName)^, AnsiString(ExtractFileName(Attachments[i])));
        end;
      end;
  ...
  begin
{$IFDEF UNICODE}
    lpRecipArray := TlpRecipArray(AnsiStrAlloc(Recipients.Count * SizeOf(TMapiRecipDesc)));
{$ELSE}
...
{$ENDIF UNICODE}
    ...
    begin
      ...
      begin
        lpRecipArray^[i].lpszName := StrCopy(new(TlpszRecipName)^, PAnsiChar(s));
        lpRecipArray^[i].lpszAddress := StrCopy(new(TlpszRecipName)^, PAnsiChar(s));
      end;
    end;
...
function SendEMailByMAPI(SenderName, SenderAddress, Subject, Body: string; Recipients, Attachments, AttachmentNames: TStrings; WithOpenMessage, ResolveNames, NeedReceipt: Boolean; intMAPISession:
  Integer): Integer;
begin
  Result := SendEMailByMAPI(AnsiString(SenderName), AnsiString(SenderAddress), AnsiString(Subject),
    AnsiString(Body), Recipients, Attachments, AttachmentNames, WithOpenMessage, ResolveNames, NeedReceipt,
    intMAPISession);
end;
{$ENDIF UNICODE}
...
begin
  ...
  begin
    try
      ReturnValue := SendEMailByMAPI(SenderName, SenderAddress, Subject, Body, Recipients, Attachments, AttachmentNames, EditDialog, ResolveNames, RequestReceipt, 0);
      case ReturnValue of
        MAPI_E_FAILURE:
          begin
            PostMessage(TWinControl(Owner).Handle, WM_SendMail, 0, 0);
